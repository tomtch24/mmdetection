...
def apply_motion_blur(image, kernel_size=5):
    ...
    Returns:
    - blurred_image: Image with motion blur applied
    """
...
def process_image(input_image_path):
    ...
    output_image_path = f"{name}_motion_blur{ext}"
...
def process_video(input_video_path):
    ...
    name, ext = os.path.splitext(base_name)
    output_video_path = f"{name}_motion_blur{ext}"
    ...
    while True:
        ...
        blurred_frame = apply_motion_blur(frame)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python apply_motion_blur.py <input_image_or_video_path>")
    else:
